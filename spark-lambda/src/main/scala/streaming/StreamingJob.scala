...
object StreamingJob {
  def main(args: Array[String]): Unit = {
    ...
    val batchDur = Seconds(4)

    def streamingApp(sc: SparkContext, bd: Duration) = {
      ...
      val activitySteam = textDStream.transform( input=>
        input.flatMap{ line =>
    ...
    val ssc = getStreamingContext(streamingApp,sc,batchDur)
